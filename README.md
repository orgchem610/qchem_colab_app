...
- **1機能追加 = 1コミット、区切りの良い段階でGitタグを打つ** 方針とします。
  最初の版は `v0.1.0`、Colab実機での動作確認で見つかった依存関係インストールの
  不具合を修正した版を `v0.1.1` としてタグを打っています(詳細はCHANGELOG.md参照)。
  「不具合修正のみ」のようにコード自体の機能は変わらない場合でも、動作に影響する
  変更(今回のrequirements.txt方針転換など)は独立したタグとして残すようにしてください。
